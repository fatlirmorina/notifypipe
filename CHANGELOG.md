...
- Fixed database initialization issue where collections were not being created
- Removed OnBeforeServe hook dependency (not needed when using PocketBase as library)
- Database collections now initialize immediately on app startup
- Fixed "no such table: \_collections" errors

### Changed

- Improved database setup with better error handling and logging
- Collections setup now runs directly after PocketBase bootstrap
